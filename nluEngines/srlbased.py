...
class Parser:
    def __init__(self):
        pass
    def qFocusIdentifier(self, dp):
        text = "null"
        doublepath = "null"
        for eojeol in dp:
            if eojeol['id'] == eojeol['head']:
                head_id = eojeol['id']
                if len(eojeol['mod']) > 1:
                    doublepath = "true"
            else:
                pass
        for eojeol in dp:
            if eojeol['label'] == "NP_SBJ" and eojeol['head'] == head_id and doublepath == "null":
                text = eojeol['text']
            else:
                pass
        if text == "null":
            for eojeol in dp:
                if eojeol['head'] == head_id and "NP_" in eojeol['label']:
                    text = eojeol['text']
                else:
                    pass
        else:
...
#        nlp = json.loads(parser.espresso(text))
...
#                pass
        ...
        stem = stemmer.stemming(text)
        qfocus = stem
        return qfocus

    def contentsIdentifier(self, dp, srl):
        affixes = [line.rstrip('\n') for line in open('./nluEngines/dictionary/KoreanAffixList.txt', 'r')]
        stemmer = requestService.Stemmer()
        contents = []
        if srl:
            for v in srl:
                ...
                for a in argument:
                    ...
                    for i in dp:
                        if i['id'] == parent_id:
                            ...
                            pass
                    if child_ids:
                        chunk_existence = True
                    else:
                        chunk_existence = False
                    if chunk_existence == True:
                        loop = True
                        while loop is True:
                            for j in dp:
                                if j['id'] in child_ids and j['label'] == "NP":
                                    chunk.append(j['text'])
                                    if j['mod']:
                                        child_ids = j['mod']
                                    else:
                                        loop = False
                                elif j['id'] in child_ids and not j['mod']:
                                    loop = False
                                else:
                                    pass
                        chunk = chunk[::-1]
                    if chunk:
                        t = " ".join(str(x) for x in chunk)
                        t = t+" "+stem
                        arg['text'] = t
                    else:
                        arg['text'] = stem
                ...
                c['qverb'] = qVerb
                c['arg'] = args
                c['content'] = qVerb+"("+args[0]['text']+","+args[1]['text']+")"
                c['qproperty'] = "undefined"
                contents.append(c)
        else:
            answer = {}
            c = {}
            ...
            for i in dp:
                if i['id'] == i['head']:
                    ...
                    pass
            for i in dp:
                if i['label'] == "NP_MOD" and i['head'] is not head_id:
                    text = i['text']
                    stem = stemmer.stemming(text)
                    if stem.endswith('의'):
                        stem = stem[:-1]
                    ...
                    c['qverb'] = "null"
                    c['arg'] = args
                    c['content'] = "null"+"("+args[0]['text']+","+args[1]['text']+")"
                    c['qproperty'] = "undefined"
                    contents.append(c)
                else:
                    c['content'] = "null"
...
'''
#for ETRI
    def qFocusIdentifier(self, morp, word_id, dp, srl, wsd, qType):
